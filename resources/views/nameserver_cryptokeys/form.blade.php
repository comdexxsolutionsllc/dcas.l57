  ...
  <div class="col-md-10">
    <select class="form-control" id="domain_id" name="domain_id" required="true">
      <option value="" style="display: none;"
              {{ old('domain_id', optional($nameserverCryptokey)->domain_id ?: '') == '' ? 'selected' : '' }} disabled
              selected>Select domain
      </option>
      @foreach ($domains as $key => $domain)
        <option
          value="{{ $key }}" {{ old('domain_id', optional($nameserverCryptokey)->domain_id) == $key ? 'selected' : '' }}>
          {{ $domain }}
        </option>
      @endforeach
    </select>
...
<div class="form-group {{ $errors->has('flags') ? 'has-error' : '' }}">
  ...
  <div class="col-md-10">
    <input class="form-control" name="flags" type="number" id="flags"
           value="{{ old('flags', optional($nameserverCryptokey)->flags) }}" min="-2147483648" max="2147483647"
    ...
    <div class="checkbox">
      <label for="active_1">
        <input id="active_1" class="" name="active" type="checkbox"
               value="1" {{ old('active', optional($nameserverCryptokey)->active) == '1' ? 'checked' : '' }}>
        Yes
      </label>
    </div>
...
<div class="form-group {{ $errors->has('content') ? 'has-error' : '' }}">
  ...
  <div class="col-md-10">
        <textarea class="form-control" name="content" cols="50" rows="10" id="content"
                  placeholder="Enter content here...">{{ old('content', optional($nameserverCryptokey)->content) }}</textarea>
