  ...
  <div class="col-md-10">
    <select class="form-control" id="domain_id" name="domain_id" required="true">
      <option value="" style="display: none;"
              {{ old('domain_id', optional($nameserverDomainmetadata)->domain_id ?: '') == '' ? 'selected' : '' }} disabled
              selected>Select domain
      </option>
      @foreach ($domains as $key => $domain)
        <option
          value="{{ $key }}" {{ old('domain_id', optional($nameserverDomainmetadata)->domain_id) == $key ? 'selected' : '' }}>
          {{ $domain }}
        </option>
      @endforeach
    </select>
...
<div class="form-group {{ $errors->has('kind') ? 'has-error' : '' }}">
  ...
  <div class="col-md-10">
    <input class="form-control" name="kind" type="text" id="kind"
           value="{{ old('kind', optional($nameserverDomainmetadata)->kind) }}" maxlength="255"
...
<div class="form-group {{ $errors->has('content') ? 'has-error' : '' }}">
  ...
  <div class="col-md-10">
        <textarea class="form-control" name="content" cols="50" rows="10" id="content"
                  placeholder="Enter content here...">{{ old('content', optional($nameserverDomainmetadata)->content) }}</textarea>
